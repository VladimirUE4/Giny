...
{ 
    ...
    { 
        ...
        public PartyMemberInformations(int lifePoints,int maxLifePoints,short prospecting,byte regenRate,short initiative,byte alignmentSide,short worldX,short worldY,double mapId,short subAreaId,PlayerStatus status,PartyEntityBaseInformation[] entities)
        {
        ...
        public override void Deserialize(IDataReader reader)
        {
            ...
            if (lifePoints < 0)
            {
                throw new Exception("Forbidden value (" + lifePoints + ") on element of PartyMemberInformations.lifePoints.");
            }

            maxLifePoints = (int)reader.ReadVarUhInt();
            if (maxLifePoints < 0)
            {
                throw new Exception("Forbidden value (" + maxLifePoints + ") on element of PartyMemberInformations.maxLifePoints.");
            }

            prospecting = (short)reader.ReadVarUhShort();
            if (prospecting < 0)
            {
                throw new Exception("Forbidden value (" + prospecting + ") on element of PartyMemberInformations.prospecting.");
            }

            regenRate = (byte)reader.ReadSByte();
            if (regenRate < 0 || regenRate > 255)
            {
                throw new Exception("Forbidden value (" + regenRate + ") on element of PartyMemberInformations.regenRate.");
            }

            initiative = (short)reader.ReadVarUhShort();
            if (initiative < 0)
            {
                throw new Exception("Forbidden value (" + initiative + ") on element of PartyMemberInformations.initiative.");
            }
            ...
            if (worldX < -255 || worldX > 255)
            {
                throw new Exception("Forbidden value (" + worldX + ") on element of PartyMemberInformations.worldX.");
            }

            worldY = (short)reader.ReadShort();
            if (worldY < -255 || worldY > 255)
            {
                throw new Exception("Forbidden value (" + worldY + ") on element of PartyMemberInformations.worldY.");
            }

            mapId = (double)reader.ReadDouble();
            if (mapId < 0 || mapId > 9.00719925474099E+15)
            {
                throw new Exception("Forbidden value (" + mapId + ") on element of PartyMemberInformations.mapId.");
            }

            subAreaId = (short)reader.ReadVarUhShort();
            if (subAreaId < 0)
            {
                throw new Exception("Forbidden value (" + subAreaId + ") on element of PartyMemberInformations.subAreaId.");
            }
