...
{ 
    ...
    { 
        ...
        public FightTeamMemberTaxCollectorInformations(short firstNameId,short lastNameId,byte level,int guildId,double uid)
        {
        ...
        public override void Deserialize(IDataReader reader)
        {
            ...
            if (firstNameId < 0)
            {
                throw new Exception("Forbidden value (" + firstNameId + ") on element of FightTeamMemberTaxCollectorInformations.firstNameId.");
            }

            lastNameId = (short)reader.ReadVarUhShort();
            if (lastNameId < 0)
            {
                throw new Exception("Forbidden value (" + lastNameId + ") on element of FightTeamMemberTaxCollectorInformations.lastNameId.");
            }

            level = (byte)reader.ReadSByte();
            if (level < 1 || level > 200)
            {
                throw new Exception("Forbidden value (" + level + ") on element of FightTeamMemberTaxCollectorInformations.level.");
            }

            guildId = (int)reader.ReadVarUhInt();
            if (guildId < 0)
            {
                throw new Exception("Forbidden value (" + guildId + ") on element of FightTeamMemberTaxCollectorInformations.guildId.");
            }

            uid = (double)reader.ReadDouble();
            if (uid < 0 || uid > 9.00719925474099E+15)
            {
                throw new Exception("Forbidden value (" + uid + ") on element of FightTeamMemberTaxCollectorInformations.uid.");
            }
