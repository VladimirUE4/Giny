...
{ 
    ...
    { 
        ...
        public virtual void Deserialize(IDataReader reader)
        {
            ...
            if (firstNameId < 0)
            {
                throw new Exception("Forbidden value (" + firstNameId + ") on element of TaxCollectorBasicInformations.firstNameId.");
            }

            lastNameId = (short)reader.ReadVarUhShort();
            if (lastNameId < 0)
            {
                throw new Exception("Forbidden value (" + lastNameId + ") on element of TaxCollectorBasicInformations.lastNameId.");
            }

            worldX = (short)reader.ReadShort();
            if (worldX < -255 || worldX > 255)
            {
                throw new Exception("Forbidden value (" + worldX + ") on element of TaxCollectorBasicInformations.worldX.");
            }

            worldY = (short)reader.ReadShort();
            if (worldY < -255 || worldY > 255)
            {
                throw new Exception("Forbidden value (" + worldY + ") on element of TaxCollectorBasicInformations.worldY.");
            }

            mapId = (double)reader.ReadDouble();
            if (mapId < 0 || mapId > 9.00719925474099E+15)
            {
                throw new Exception("Forbidden value (" + mapId + ") on element of TaxCollectorBasicInformations.mapId.");
            }

            subAreaId = (short)reader.ReadVarUhShort();
            if (subAreaId < 0)
            {
                throw new Exception("Forbidden value (" + subAreaId + ") on element of TaxCollectorBasicInformations.subAreaId.");
            }
