...
{ 
    ...
    { 
        ...
        public virtual void Deserialize(IDataReader reader)
        {
            ...
            if (objectUID < 0)
            {
                throw new Exception("Forbidden value (" + objectUID + ") on element of BidExchangerObjectInfo.objectUID.");
            }

            objectGID = (short)reader.ReadVarUhShort();
            if (objectGID < 0)
            {
                throw new Exception("Forbidden value (" + objectGID + ") on element of BidExchangerObjectInfo.objectGID.");
            }

            objectType = (int)reader.ReadInt();
            if (objectType < 0)
            {
                throw new Exception("Forbidden value (" + objectType + ") on element of BidExchangerObjectInfo.objectType.");
            }
