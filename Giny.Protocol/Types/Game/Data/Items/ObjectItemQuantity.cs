...
{ 
    ...
    { 
        ...
        public override void Deserialize(IDataReader reader)
        {
            ...
            if (objectUID < 0)
            {
                throw new Exception("Forbidden value (" + objectUID + ") on element of ObjectItemQuantity.objectUID.");
            }

            quantity = (int)reader.ReadVarUhInt();
            if (quantity < 0)
            {
                throw new Exception("Forbidden value (" + quantity + ") on element of ObjectItemQuantity.quantity.");
            }
