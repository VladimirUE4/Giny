...
{ 
    ...
    { 
        ...
        public override void Deserialize(IDataReader reader)
        {
            ...
            if (position < 0)
            {
                throw new Exception("Forbidden value (" + position + ") on element of ObjectItem.position.");
            }

            objectGID = (short)reader.ReadVarUhShort();
            if (objectGID < 0)
            {
                throw new Exception("Forbidden value (" + objectGID + ") on element of ObjectItem.objectGID.");
            }
            ...
            if (objectUID < 0)
            {
                throw new Exception("Forbidden value (" + objectUID + ") on element of ObjectItem.objectUID.");
            }

            quantity = (int)reader.ReadVarUhInt();
            if (quantity < 0)
            {
                throw new Exception("Forbidden value (" + quantity + ") on element of ObjectItem.quantity.");
            }
