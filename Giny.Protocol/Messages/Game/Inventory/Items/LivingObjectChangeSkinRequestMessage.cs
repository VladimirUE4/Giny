...
{ 
    ...
    { 
        ...
        public override void Serialize(IDataWriter writer)
        {
            ...
            if (skinId < 0)
            {
                throw new Exception("Forbidden value (" + skinId + ") on element skinId.");
            }
        ...
        public override void Deserialize(IDataReader reader)
        {
            ...
            if (livingUID < 0)
            {
                throw new Exception("Forbidden value (" + livingUID + ") on element of LivingObjectChangeSkinRequestMessage.livingUID.");
            }

            livingPosition = (byte)reader.ReadSByte();
            if (livingPosition < 0 || livingPosition > 255)
            {
                throw new Exception("Forbidden value (" + livingPosition + ") on element of LivingObjectChangeSkinRequestMessage.livingPosition.");
            }

            skinId = (int)reader.ReadVarUhInt();
            if (skinId < 0)
            {
                throw new Exception("Forbidden value (" + skinId + ") on element of LivingObjectChangeSkinRequestMessage.skinId.");
            }
