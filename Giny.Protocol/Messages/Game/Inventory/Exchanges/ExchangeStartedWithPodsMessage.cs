...
{ 
    ...
    { 
        public new const ushort Id = 4599;
        ...
        public override void Serialize(IDataWriter writer)
        {
            ...
            {
                throw new Exception("Forbidden value (" + firstCharacterId + ") on element firstCharacterId.");
            }

            writer.WriteDouble((double)firstCharacterId);
            if (firstCharacterCurrentWeight < 0)
            {
                throw new Exception("Forbidden value (" + firstCharacterCurrentWeight + ") on element firstCharacterCurrentWeight.");
            }

            writer.WriteVarInt((int)firstCharacterCurrentWeight);
            if (firstCharacterMaxWeight < 0)
            {
                throw new Exception("Forbidden value (" + firstCharacterMaxWeight + ") on element firstCharacterMaxWeight.");
            }
            ...
            {
                throw new Exception("Forbidden value (" + secondCharacterId + ") on element secondCharacterId.");
            }

            writer.WriteDouble((double)secondCharacterId);
            if (secondCharacterCurrentWeight < 0)
            {
                throw new Exception("Forbidden value (" + secondCharacterCurrentWeight + ") on element secondCharacterCurrentWeight.");
            }

            writer.WriteVarInt((int)secondCharacterCurrentWeight);
            if (secondCharacterMaxWeight < 0)
            {
                throw new Exception("Forbidden value (" + secondCharacterMaxWeight + ") on element secondCharacterMaxWeight.");
            }
        ...
        public override void Deserialize(IDataReader reader)
        {
            ...
            if (firstCharacterId < -9.00719925474099E+15 || firstCharacterId > 9.00719925474099E+15)
            {
                throw new Exception("Forbidden value (" + firstCharacterId + ") on element of ExchangeStartedWithPodsMessage.firstCharacterId.");
            }

            firstCharacterCurrentWeight = (int)reader.ReadVarUhInt();
            if (firstCharacterCurrentWeight < 0)
            {
                throw new Exception("Forbidden value (" + firstCharacterCurrentWeight + ") on element of ExchangeStartedWithPodsMessage.firstCharacterCurrentWeight.");
            }

            firstCharacterMaxWeight = (int)reader.ReadVarUhInt();
            if (firstCharacterMaxWeight < 0)
            {
                throw new Exception("Forbidden value (" + firstCharacterMaxWeight + ") on element of ExchangeStartedWithPodsMessage.firstCharacterMaxWeight.");
            }

            secondCharacterId = (double)reader.ReadDouble();
            if (secondCharacterId < -9.00719925474099E+15 || secondCharacterId > 9.00719925474099E+15)
            {
                throw new Exception("Forbidden value (" + secondCharacterId + ") on element of ExchangeStartedWithPodsMessage.secondCharacterId.");
            }

            secondCharacterCurrentWeight = (int)reader.ReadVarUhInt();
            if (secondCharacterCurrentWeight < 0)
            {
                throw new Exception("Forbidden value (" + secondCharacterCurrentWeight + ") on element of ExchangeStartedWithPodsMessage.secondCharacterCurrentWeight.");
            }

            secondCharacterMaxWeight = (int)reader.ReadVarUhInt();
            if (secondCharacterMaxWeight < 0)
            {
                throw new Exception("Forbidden value (" + secondCharacterMaxWeight + ") on element of ExchangeStartedWithPodsMessage.secondCharacterMaxWeight.");
            }
